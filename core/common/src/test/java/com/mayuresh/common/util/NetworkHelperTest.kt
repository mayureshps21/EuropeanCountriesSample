...
class NetworkHelperTest {
    ...
    fun `isNetworkConnected should return true when SDK is above M and connected with TRANSPORT_WIFI`() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            `when`(mockConnectivityManager.getNetworkCapabilities(mockNetwork)).thenReturn(
                mockNetworkCapabilities
            )
            `when`(mockNetworkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)).thenReturn(
                true
            )
    ...
    fun `isNetworkConnected should return true when SDK is above M and connected with TRANSPORT_CELLULAR`() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            `when`(mockNetworkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR)).thenReturn(
                true
            )
    ...
    fun `isNetworkConnected should return true when SDK is above M and connected with TRANSPORT_ETHERNET`() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            `when`(mockNetworkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_ETHERNET)).thenReturn(
                true
            )
    ...
    fun `isNetworkConnected should return false when SDK is above M and connected with other transport`() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            `when`(mockNetworkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_USB)).thenReturn(
                true
            )
